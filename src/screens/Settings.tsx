...
import { encryptBackup, decryptBackup, parseImport, type EncryptedEnvelope } from '../lib/backup'
...
export default function Settings() {
  ...
  async function exportPlain() {
    ...
    download(`brain-dump-${stamp()}.json`, JSON.stringify(data, null, 2))
    flash('Backup saved. Drop it in iCloud Drive to keep it safe.')
  }

  async function exportEncrypted() {
    if (passphrase.length < 4) {
      setErr('Choose a passphrase of at least 4 characters.')
      return
    }
    const data = await exportAll(Date.now())
    const env = await encryptBackup(data, passphrase)
    download(`brain-dump-${stamp()}.encrypted.json`, JSON.stringify(env, null, 2))
    setPassphrase('')
    flash('Encrypted backup saved. Keep your passphrase somewhere safe.')
  }
      ...
      <div className="flex-1 space-y-8 overflow-y-auto pb-4">
        <section>
          ...
          <input
            ...
            onBlur={() => name !== null && updateName(name)}
          />
        </section>
        ...
        <section>
          ...
          {pendingEnc ? (
            <div className="card space-y-3">
              ...
              <div className="flex gap-3">
                <button className="btn-primary flex-1" onClick={unlockAndImport}>
                  Unlock & restore
                </button>
          ...
          <p className="mt-2 text-xs text-mist-500">Restoring replaces what's currently on this device.</p>
        </section>

        {(msg || err) && (
          <p className={`rounded-2xl p-4 text-sm ${err ? 'bg-red-500/15 text-red-200' : 'bg-nebula-500/15 text-nebula-200'}`}>
            {err ?? msg}
          </p>
        )}

        <section className="pt-2">
          <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-mist-500">Danger zone</p>
          {confirmWipe ? (
            <div className="flex items-center gap-3">
              <button className="btn-primary flex-1 bg-red-500 shadow-none" onClick={doWipe}>
                Erase everything
              </button>
